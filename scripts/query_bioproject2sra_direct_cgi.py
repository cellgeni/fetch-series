...
async def sra_cgi_fetch_runinfo(
    get: ThrottledGet,
    webenv: str,
    query_key: str,
) -> str:
    base = "https://trace.ncbi.nlm.nih.gov/Traces/sra-db-be/sra-db-be.cgi"
    params = {
        "WebEnv": webenv,
        "rettype": "runinfo",
        "query_key": query_key,
...
async def bioproject2sra(
    ...
    async def _run():
        ...
        return await sra_cgi_fetch_runinfo(
            get=get,
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        results = await tqdm_asyncio.gather(
            *tasks, desc="Fetching BioProject→SRA (direct-cgi)"
        )
    ...
    output_path = os.path.join(
        OUTPUT_DIR, "results", "bioproject2sra_direct_cgi_summary.csv"
    )
